...
public class DbData {
	...
	public static ArrayList<ProgramBean> getProgramBeansHot() {
		...
		Cursor cursor = db
				.rawQuery(
						"select * from program where id=20110213 or id=20110217 or id=20110221 or id=20110224 or id=20110410 or id=20110420",
						null);
		while (cursor.moveToNext()) {
			ProgramBean programBean = new ProgramBean();
			programBean.setId(cursor.getString(0));
	...
	public static ArrayList<ProgramBean> getProgramBeansAgo() {
		...
		while (cursor.moveToNext()) {
			...
			programBean.setPlayTime(cursor.getInt(3));
			programBean.setNum((cursor.getInt(4)));
			programBean.image = new BitmapDrawable(getImageFromAssetsFile("img"
					+ programBean.getId() + ".jpg"));
			programBeans.add(programBean);
		}
		return programBeans;
	}
	...
	public static boolean createFile() {
		if (Environment.getExternalStorageState().equals(
			...
			if (fileCache == null || !fileCache.exists()) {
				fileCache = new File(Environment.getExternalStorageDirectory()
						.toString() + "/lecture");
				fileCache.mkdir();
			}
			// 创建观看记录文本文件
	...
	public static void writeHistory(HistoryBean h) {
		...
		for (int i = 0; i < historyBeans.size(); i++) {
			...
			if (historyBean.getId().equals(h.getId())
					&& historyBean.getEpisode().equals(h.getEpisode())) {
