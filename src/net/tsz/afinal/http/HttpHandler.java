...
public class HttpHandler<T> extends AsyncTask<Object, Object, Object> implements EntityCallBack {
	...
	private void handleResponse(HttpResponse response) {
		...
		System.out.println("status.getStatusCode():" + status.getStatusCode());
		if (headers != null) {
			for (Header h : headers) {
				System.out.println("getName:" + h.getName() + " getName:" + h.getValue());
			}
		}
		...
		if (status.getStatusCode() >= 300) {
			...
			try {
				if (targetUrl != null && !targetUrl.contains(BaseApi.ONLINE)) {
					Header head = response.getFirstHeader("Content-Type");
					if (head != null && head.getValue() != null && head.getValue().equals("text/html")) {
						publishProgress(UPDATE_FAILURE, new HttpResponseException(status.getStatusCode(), status.getReasonPhrase()), status.getStatusCode(), "下载失败");
						return;
					}
				}
